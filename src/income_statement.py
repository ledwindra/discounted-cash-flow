...
def get_company():
    """
    Returns file name for income statement (coded as "3").
    """

    company_list = [x for x in os.listdir('financial-statements/') if x.endswith('{}-Audit-3.csv'.format(YEAR))]
...
def get_column():
    ...
    column_name = dict({
        'Penjualan dan pendapatan usaha': 'revenues',
        'Beban pokok penjualan dan pendapatan': 'cost_of_goods_sold',
        ...
        'Laba (rugi) dari operasi yang dihentikan': 'after_tax_income_nonoperational',
        'Jumlah laba (rugi)': 'after_tax_income',
        'Jumlah laba rugi komprehensif': 'comprehensive_income',
        'Laba (rugi) per saham dasar dari operasi yang dilanjutkan': 'income_per_share',
...
if __name__ == "__main__":
    ...
    df.to_csv(os.getcwd() + '/sample-output/income-statement-{}.csv'.format(YEAR), index=False)